...
from .models import ElevationData, ElevationRequest, ElevationResponse, ElevationResult
...
def _check_api_response(data: dict[str, Any]) -> None:
    ...
    if data.get("status") != "OK":
        ...
        msg = f"Elevation API error: {error_msg}"
...
class OpenTopoDataElevationService:
    ...
    async def get_elevation(self, request: ElevationRequest) -> ElevationResponse:
        ...
        try:
            ...
            msg = f"Failed to get elevation for ({request.latitude}, {request.longitude}): {e!s}"
            raise httpx.HTTPError(msg) from e
        except (ValueError, KeyError) as e:
            # Handle JSON parsing errors
            msg = f"Invalid response format from elevation API: {e!s}"
...
_elevation_service: OpenTopoDataElevationService | None = None
